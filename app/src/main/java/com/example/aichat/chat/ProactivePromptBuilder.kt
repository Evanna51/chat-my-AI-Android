...
- split: 把这条回复拆成几条独立短消息逐条发出（像真人发短信一样），最多5段、每段≥8字。
  以下情况 **应该用 split**：先抛情绪/感叹再补说明 / 有自然停顿节拍 / 想法分两步走。
  以下情况 **不用 split**：单句话 / 只是列举多个问题 / 分段会破坏语气连贯性。
...
- 只想说一句 → split=null, 正常单条输出.
- 想分两三拍 → 用 META 的 split 字段传各段, 不要把不同话题塞进一个 split.
末尾追加 META. follow-up 后通常 autoStop=true.
